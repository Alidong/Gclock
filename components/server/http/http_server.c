 ...
 */
static esp_err_t cpuinfo_put_handler(httpd_req_t *req)
{
    ...
    while (remaining > 0) {
        ...
        cJSON *FullSystemData = cJSON_GetObjectItem(root, "FullSystemData");  /*FullSystemData 的键值对为数组，*/ 
        cJSON *cpuData=cJSON_GetObjectItem(FullSystemData,"cpu");
        item=cJSON_GetObjectItem(cpuData,"cpu_name");
        printf("cjson=%s\r\n",cJSON_Print(item));
        cJSON_Delete(root);
    }
...
static const httpd_uri_t cpuInfo = {
    .uri       = "/cpuinfo",
    .method    = HTTP_PUT,
    .handler   = cpuinfo_put_handler,
    .user_ctx  = NULL
};
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &cpuInfo);
        #if CONFIG_EXAMPLE_BASIC_AUTH
...
static void http_server(void* xTimer)
{
    while (1)
    {
        if (wifi_is_connect() && !http_server_ctl.running)
        {
            ESP_LOGI(TAG,"start");
            http_server_ctl.server = start_webserver();
            http_server_ctl.running=true;
        }
        else if(!wifi_is_connect() && http_server_ctl.running)
        {
